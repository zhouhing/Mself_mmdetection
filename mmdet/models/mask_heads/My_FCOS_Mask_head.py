...
class FCOSMaskHead(nn.Module):
    ...
    def __init__(self,
        ...
        # for level i==3: ...
        for i in range(1, self.end_level + 1): ## i=1,2
    ...
    def forward(self, feats, obj_reps, img_metas):
        print("RdsMaskHead forward funcation!")
        print("RdsMaskHead 接受的数据：",type(feats),feats[0].size(),len(feats))
        print(len(obj_reps))
        print(obj_reps[0].size(),obj_reps[1].size())
        print(img_metas)
        x = feats[0] # 取分辨率最大的特征图，即第一层特征图
        ...
        for i, (pixel_rep, obj_rep, img_meta) in enumerate(zip(pixel_reps, obj_reps, img_metas)):
            ...
            # TODO　关联模块，两者进行点积运算，应该是*吧，源码写错了？
            pred_masks = pixel_rep @ obj_rep.t() # 关联模块，两者进行点积运算
            # pred_masks = pixel_rep * obj_rep.t()# 应该是*吧，源码写错了？
        ...
        print("pred_masks_list:",len(pred_masks_list),pred_masks_list[0].size())
        return pred_masks_list

    @force_fp32(apply_to=('mask_pred', ))
    def loss(self, pred_masks, bboxes, gt_masks, img_metas, cfg):
        print('RdsMaskHead loss funcation!')
        # print("pred_masks:",pred_masks[0].size(),pred_masks[1].size(),len(pred_masks))
        # print("bboxes:",bboxes[0].size(),bboxes[1].size(),len(bboxes))
        # print("gt_masks:",gt_masks[0].size(),gt_masks[1].size())
        # print("img_metas:",len(img_metas),img_metas[0],'\n',img_metas[1])
        # print("cfg:",cfg)
    ...
    def mask_ohem_loss(self, pre_loss, gt_mask, crop_mask, pos_neg_ratio):
        print("mask_ohem_loss funcation!")
    ...
    def crop(self, masks, bboxes, cfg):
        print("crop funcation!")
        print(cfg)
        print("masks:",masks.size())
        n, h, w = masks.size()[:3]# 获取mask像素点的个数，和像素点的位置
        print("n.h.w:",n,h,w)
        print(bboxes[:,0],bboxes[:,2])
        print("~~~~~~~~~~~~~~~"*10)
        x1, x2 = sanitize_coordinates(bboxes[:, 0], bboxes[:, 2], w, cfg['padding'], cast=True) # 按padding=0.25裁剪bboxes
        ...
        print("masks:",masks_left,masks_left.size())
        crop_mask = masks_left * masks_right * masks_up * masks_down
        print("crop_mask：",crop_mask.size())
        return crop_mask

    def get_masks(self, mask_preds, bbox_preds, img_metas, cfg, rescale=False):
        print("get_masks funcation!")
        mask_pred_list = []
