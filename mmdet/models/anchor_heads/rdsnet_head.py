...
class RdsnetHead(AnchorHead):
    ...
    def __init__(self,
        ...
        print("RdsnetHead init funcation!")
    ...
    def _init_layers(self):
        print('RdsnetHead init_layer funcation!')
    ...
    def forward_single(self, x):
        print("RdsnetHead forward_single funcation!")
    ...
    def forward(self, feats):
        print("RdsnetHead forward funcation ! ")
        print(len(feats))
        for i in range(len(feats)):
            cls_score, bbox_pred, obj_rep=self.forward_single(feats[i])
            print("self.forwardsingle out:",cls_score.size(),bbox_pred.size(),obj_rep.size())
            print("features{}:".format(i),feats[i].size())
    ...
    def get_anchors(self, featmap_sizes, img_metas):
        ...
        """
        print("RdsnetHead get_anchors funcation!")
        ...
        for i in range(num_levels):
            anchors = self.anchor_generators[i].grid_anchors(
                featmap_sizes[i], self.anchor_strides[i]) # 获得features_map上映射回原图上的所有的anchor点网格
            multi_level_anchors.append(anchors)
        anchor_list = [multi_level_anchors for _ in range(num_imgs)]

        # for each image, we compute valid flags of multi level anchors
        # 对于每个图像，我们计算多级锚点的有效标志
        # 获取每个图像上所有像素点的anchor的bool标记
        valid_flag_list = []
        for img_id, img_meta in enumerate(img_metas):
            ...
            for i in range(num_levels):
                ...
                valid_feat_h = min(int(np.ceil(h / anchor_stride)), feat_h) # np.ceil() 表示向上取整，并映射到feature_map中，与feat_h进行大小的对比，取最小值
                valid_feat_w = min(int(np.ceil(w / anchor_stride)), feat_w)
                flags = self.anchor_generators[i].valid_flags(
                    (feat_h, feat_w), (valid_feat_h, valid_feat_w)) # valid_flags就是将在真实图片像素区域内的像素点对应位置的值全置为True，外部扩充的置为False
    ...
    def loss_single(self, cls_score, bbox_pred, labels, label_weights,
        ...
        # classification loss
        print("RdsnetHead loss_single funcation!")
    ...
    def loss(self,
        ...
        print("RdsnetHead loss funcation!")
        print("cls_scores:",len(cls_scores))
        print("bbox_preds:",len(bbox_preds))
        print("obj_reps:",len(obj_reps),obj_reps[0].size(),obj_reps[1].size(),obj_reps[2].size(),obj_reps[3].size(),obj_reps[4].size())
        ...
        anchor_list, valid_flag_list = self.get_anchors(
            featmap_sizes, img_metas) # 获得feature_map上所有的anchor和所有图片上有效区域的anchor（features_map上的anchors）
    ...
    def get_bboxes(self, cls_scores, bbox_preds, obj_reps, img_metas, cfg,
                   rescale=False):
        print("RdsnetHead get_bboxes funcation!")
    ...
    def get_bboxes_single(self,
                          ...
                          cfg,
                          rescale=False):
        print("RdsnetHead get_bboxes_single funcation!")
        # TODO:
