...
class MyMaskFCOSHead(nn.Module):
    ...
    def __init__(self,
        ...
        super(MyMaskFCOSHead, self).__init__()
        print("__init__ funcation!"*5)
    ...
    def _init_layers(self):
        print("_init_layers funcation!"*5)
        ...
        ############# 添加Mask分支 ###############
        ############# 添加Mask分支 ###############
        self.fcos_mask=nn.ConvTranspose2d(self.feat_channels,self.feat_channels,kernel_size=2,stride=2,padding=1)
        self.mask_conv = nn.Conv2d(256, self.cls_out_channels, kernel_size=1, stride=1)
        self.relu=nn.ReLU()
        self.sigmoid=nn.Sigmoid()
        self.scales = nn.ModuleList([Scale(1.0) for _ in self.strides])

    def init_weights(self):
        print("init_weights funcation!"*5)
    ...
    def forward(self, feats):
        print("forward funcation!"*5)
        return multi_apply(self.forward_single, feats, self.scales)

    def forward_single(self, x, scale):
        print("forward_simgle funcation!"*5)
        cls_feat = x
        reg_feat = x
        print(x.size())
        ...
        ############  mask分支  #########
        mask_feats = self.fcos_mask(cls_feat)
        mask_feats = self.relu(mask_feats)
        mask_feats = self.mask_conv(mask_feats)
        mask_feats = self.sigmoid(mask_feats)
    ...
    def loss(self,
        ...
        # print("labels:",labels.shape,"bbox_targets:",bbox_targets.shape)
        ...
        flatten_points = torch.cat(
            [points.repeat(num_imgs, 1) for points in all_level_points])##重复两次生成两张图的点，cat到一起
        ...
        pos_bbox_preds = flatten_bbox_preds[pos_inds]# 像素点的label不为0的坐标对应的预测bbox的值
    ...
    def fcos_target(self, points, gt_bboxes_list, gt_labels_list):
        ...
        # print("expanded_regress_ranges:", expanded_regress_ranges,np.array( expanded_regress_ranges).shape)
        # concat all levels points and regress ranges
        concat_regress_ranges = torch.cat(expanded_regress_ranges, dim=0)
        # print("concat_regress_ranges:", concat_regress_ranges, concat_regress_ranges.size())
        concat_points = torch.cat(points, dim=0)
        # print("fcos_points:", concat_points, concat_points.size())
    ...
    def fcos_target_single(self, gt_bboxes, gt_labels, points, regress_ranges):
        print('fcos_target_single funcation!' * 5,)
        ...
        areas = areas[None].repeat(num_points,1) ##  - *sizes (torch.Size ot int...)-沿着每一维重复的次数,给每个点分配面积
        ...
        gt_bboxes = gt_bboxes[None].expand(num_points, num_gts, 4) # 对应于每个点的，每个类别的真实区域
        # print("gt_bboxes:",gt_bboxes)
        # print("points:",points,points.size())
        xs, ys = points[:, 0], points[:, 1] # 获得点的 横纵坐标
        ...
        min_area, min_area_inds = areas.min(dim=1)  # 当这个像素点生成的框属于多个类别时，将这个框分配给生成框面积小的实例，
                                                    ...
                                                    # min_area_inds代表的是在该个像素点的位置生成的对应几个类别的框中，选取的面积最小的边框的下标（在所有类别中的位置下标）
        ...
        labels = gt_labels[min_area_inds] # 获取对应最小面积的类别标签
...
##################################################################################################################
    ...
    def get_bboxes_single(self,
                          ...
                          cfg,
                          rescale=False):
        print('get_bboxes_single funcation!' * 5, '\n')
